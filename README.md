...
翌営業日の注目経済指標、および Gemini が生成する日本語サマリーをメール配信する個人プロジェクト。

## 配信内容

- 本日のサマリー(Gemini + Google Search で生成、約500字)
- 為替: USD/JPY, EUR/USD, GBP/USD
- 金利: UST 5Y / 10Y / 30Y
- コモディティ: WTI, Brent, Gold
- 株価指数: S&P 500, Nikkei 225
- 翌営業日の注目経済指標(G10 通貨, High/Medium)
...
2. 1日1回の運用なら無料枠(grounded query 500件/日)に余裕で収まる
...
| `GEMINI_API_KEY` | 上で取得した Gemini API キー(未設定ならサマリーをスキップ) |
...
# サマリーをスキップして数値だけ(LLM の課金/クォータを気にしないため)
python -m recap --dry-run --no-summary
...
`config/prompt.md` を編集すれば、コード変更なしでプロンプトを差し替えられます。
編集後 `python -m recap --dry-run` で出力を確認 → 気に入ったらコミット。
...
recap/
  ...
  llm.py        # Gemini API + Google Search Grounding
  format.py     # メール本文整形
